...
int createSocket(uint16_t port)
{
    ...
    if (socket_fd<0)
    {
        sprintf(log_msg, "Server: error al crar flujo en socket => %s\n", strerror(errno));
        log(log_msg);
        return -1;
    }
    ...
    {
        sprintf(log_msg, "Server: error en enlace socket => %s\n", strerror(errno));
        log(log_msg);
        return -1;
    }
    ...
    listen(socket_fd,5);
    sprintf(log_msg, "Server: Escuchando en puerto %d\n", port);
    log(log_msg);
...
int waitForClient(int socket_fd, int protocol_type)
{
    ...
    sprintf(log_msg, "Server: esperando un nuevo cliente...\n");
    log(log_msg);
...
void *handleConnections(void *arguments)
{
    ...
    sprintf(log_msg, "Server: Hilo creado para la identificacion del cliente: %d\n", client_fd);
    log(log_msg);
    ...
    while(*run_server)
    {
        ...
        if (messageSize <= 0 || messageSize > NET_BUFFER_SIZE)
        {
            ...
            if (messageSize == 0)
            {
                sprintf(log_msg, "Modbus Server: cliente ID: %d ha cerrado la conexion\n", client_fd);
                log(log_msg);
            }
            else
            {
                sprintf(log_msg, "Modbus Server: Algo esta mal con el ID del cliente: %d Tamanyo del mensaje : %i\n", client_fd, messageSize);
                log(log_msg);
            }
            break;
        }
    ...
    close(client_fd);
    sprintf(log_msg, "Finalizado el hilo de conexiones Modbus\r\n");
    log(log_msg);
    pthread_exit(NULL);
}
...
void startServer(uint16_t port, int protocol_type)
{
    ...
    while(*run_server)
    {
        ...
        if (client_fd < 0)
        {
            sprintf(log_msg, "Server: Error al aceptar cliente!\n");
            log(log_msg);
        }
        ...
        else
        {
            ...
            pthread_t thread;
            int ret = -1;
            sprintf(log_msg, "Server: Cliente aceptado! Crear hilo para nuevo cliente con ID: %d...\n", client_fd);
            log(log_msg);
    ...
    close(socket_fd);
    close(client_fd);
    sprintf(log_msg, "Finalizado el hilo del servidor\r\n");
    log(log_msg);
}
