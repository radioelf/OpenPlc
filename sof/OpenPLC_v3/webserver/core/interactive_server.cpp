...
int createSocket_interactive(int port)
{
    ...
    {
        sprintf(log_msg, "Servidor interactivo: Error en enlace socket => %s\n", strerror(errno));
        log(log_msg);
        exit(1);
    }
    // we accept max 5 pending connections
    listen(socket_fd,5);
    sprintf(log_msg, "Servidor interactivo: escuchar el puerto %d\n", port);
    log(log_msg);
...
//-----------------------------------------------------------------------------
// Subproceso para manejar solicitudes para cada cliente conectado
//-----------------------------------------------------------------------------
