...
void parseConfig()
{
    ...
    else
    {
        ...
        sprintf(log_msg, "Saltar la configuracion de dispositivos esclavos (mbconfig.cfg fichero NO encontrado)\n");
        log(log_msg); 
    }
...
void *querySlaveDevices(void *arg)
{
    while (run_openplc)
    {
        ...
        for (int i = 0; i < num_devices; i++)
        {
            ...
            {
                sprintf(log_msg, "Dispositivo %s esta desconectado. Intentando reconectar...\n", mb_devices[i].dev_name);
                log(log_msg);
                if (modbus_connect(mb_devices[i].mb_ctx) == -1)
                {
                    sprintf(log_msg, "La conexion fallo en el dispositivo MB %s: %s\n", mb_devices[i].dev_name, modbus_strerror(errno));
                    log(log_msg);
                ...
                {
                    sprintf(log_msg, "Conectando con el dispositivo MB %s\n", mb_devices[i].dev_name);
                    log(log_msg);
            ...
            if (mb_devices[i].isConnected)
            {
                struct timespec ts;
                ts.tv_sec = 0;
                //ts.tv_nsec = (1000*1000*1000*28)/mb_devices[i].rtu_baud; //28000000000 nsec/19200 =1,45ms
                ts.tv_nsec = (1000*1000*1000*48)/mb_devices[i].rtu_baud; //48000000000 nsec/19200 =2,5ms
                ...
                if (mb_devices[i].discrete_inputs.num_regs != 0)
                {
                    ...
                    if (return_val == -1)
                    {
                        ...
                        sprintf(log_msg, "Modbus FALLO en la lectura de las entradas del dispositivo MB %s: %s\n", mb_devices[i].dev_name, modbus_strerror(errno));
                        log(log_msg);
                        bool_input_index += (mb_devices[i].discrete_inputs.num_regs);
                        if (special_functions[2] != NULL) *special_functions[2]++;
                    }
                ...
                if (mb_devices[i].coils.num_regs != 0)
                {
                    ...
                    if (return_val == -1)
                    {
                        ...
                        sprintf(log_msg, "Modbus FALLO en la escritura de las bobinas del dispositivo MB %s: %s\n", mb_devices[i].dev_name, modbus_strerror(errno));
                        log(log_msg);
                        if (special_functions[2] != NULL) *special_functions[2]++;
                    }
                ...
                if (mb_devices[i].holding_read_registers.num_regs != 0)
                {
                    ...
                    if (return_val == -1)
                    {
                        ...
                        sprintf(log_msg, "Modbus FALLO en la lectura de los registros Holding del dispositivo MB %s: %s\n", mb_devices[i].dev_name, modbus_strerror(errno));
                        log(log_msg);
                        int_input_index += (mb_devices[i].holding_read_registers.num_regs);
                        if (special_functions[2] != NULL) *special_functions[2]++;
                    }
                ...
                if (mb_devices[i].holding_registers.num_regs != 0)
                {
                    ...
                    if (return_val == -1)
                    {
                        ...
                        sprintf(log_msg, "Modbus FALLO en la escritura de los registros Holding en el dispositivo MB %s: %s\n", mb_devices[i].dev_name, modbus_strerror(errno));
                        log(log_msg);
                        if (special_functions[2] != NULL) *special_functions[2]++;
                    }
