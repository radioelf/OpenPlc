    ...
    return_str = pages.w3_style + pages.dashboard_head + draw_top_div() + """
            <div class='main'>
                <div class='w3-sidebar w3-bar-block' style='width:250px; background-color:#3D3D3D'>
                    ...
                    <div class="w3-bar-item"><img src='/static/restart-icon-64x64.png' alt='Reboot' style='width:47px;height:32px;padding:0px 15px 0px 0px;float:left'><p style='font-family:\"Roboto\", sans-serif; font-size:20px; color:white;margin: 2px 0px 0px 0px'>Reboot PI</p></div>
                    <div class="w3-bar-item"><img src='/static/shutdown-icon-64x64.png' alt='Shutdown' style='width:47px;height:32px;padding:0px 15px 0px 0px;float:left'><p style='font-family:\"Roboto\", sans-serif; font-size:20px; color:white;margin: 2px 0px 0px 0px'>Power OFF</p></div>
                    <br>
                    <br>
                </div>
...
def dashboard():
    ...
    if (flask_login.current_user.is_authenticated == False):
        ...
        return_str += """
            <div class='main'>
                <div class='w3-sidebar w3-bar-block' style='width:250px; background-color:#3D3D3D'>
                    ...
                    <a href='reboot' title="ATENCION!! STOP OpenPLC y se reiniciara" class='w3-bar-item w3-button'><img src='/static/restart-icon-64x64.png' alt='Reboot' style='width:47px;height:32px;padding:0px 15px 0px 0px;float:left'><p style='font-family:\"Roboto\", sans-serif; font-size:20px; color:white;margin: 2px 0px 0px 0px'>Reboot PI</p></a>
                    <a href='shutdown' title="ATENCION!! OpenPLC se apagara completamente" class='w3-bar-item w3-button'><img src='/static/shutdown-icon-64x64.png' alt='Shutdown' style='width:47px;height:32px;padding:0px 15px 0px 0px;float:left'><p style='font-family:\"Roboto\", sans-serif; font-size:20px; color:white;margin: 2px 0px 0px 0px'>Power OFF</p></a>
                    <br>
                    <br>"""
        return_str += draw_status()
        return_str += """
        ...
        return_str += "<p style='font-family:'Roboto', sans-serif; font-size:16px'><b>Clock:</b> " + time.strftime("%X") +  "</p>"
        with open('./scripts/openplc_driver') as f: openplc_driver = f.read().rstrip()
        return_str += "<p style='font-family:'Roboto', sans-serif; font-size:16px'><b>Driver:</b> " + openplc_driver + "</p>"
        ...
        return_str += """
            ...
            <div style="margin-left:320px; margin-right:70px">
                <div style="w3-container">
                    ...
                    <p><b>Atencion:</b> Los dispositivos esclavos se conectan a partir de la direccion 100 (i.e. %IX100.0, %IW100, %QX100.0, y %QW100)
                    <table>
                        <tr style='background-color: white'>
...
def monitoring():
    #print("Estado actual OpenPLC: " + openplc_runtime.status())
    if (flask_login.current_user.is_authenticated == False):
        return flask.redirect(flask.url_for('login'))
    else:
        if (openplc_runtime.status() == "Compiling"): return draw_compiling_page()
...
@app.route('/reboot')
def reboot():
	if (openplc_driver == "rpi_zero"):
		openplc_runtime.stop_runtime()
		time.sleep(1)
		monitor.stop_monitor()
		os.system("shutdown -r now")
	else:
	    return 'Driver NOT valid'
